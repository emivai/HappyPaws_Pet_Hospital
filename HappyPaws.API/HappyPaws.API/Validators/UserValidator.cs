...
namespace HappyPaws.API.Validators
{
    public class CreateUserValidator : AbstractValidator<CreateUserDTO>
    {
        ...
        {
            RuleFor(customer => customer.Type).NotNull().IsInEnum().WithMessage("Type invalid. Valid user type values are: 0 (admin), 1 (client) and 2 (doctor).");

            RuleFor(customer => customer.Name).NotNull().NotEmpty().WithMessage("Name is required.");
            ...
            RuleFor(customer => customer.Surname).NotNull().NotEmpty().WithMessage("Surname is required.");
            ...
            RuleFor(customer => customer.Email).NotNull().NotEmpty().WithMessage("Email is required.");
            RuleFor(customer => customer.Email).Length(1, 50).WithMessage("Email has to be 1-50 characters long.");

            RuleFor(customer => customer.PhoneNumber).NotNull().NotEmpty().WithMessage("Phone number is required and must be 1-15 characters long.");
